...
class TestA1Functions(unittest.TestCase):
    ...
    def test_is_multiple_of_5(self):
        ...
        test_cases = [
            (0, True), (135, True), (1, False), (22, False), (-1, False),
            (-15, True),
        ]
        self._test_function_on_cases(a1.is_multiple_of_5, test_cases)
    ...
    def test_five_times_list(self):
        """Provided tests for five_times_list in starter code."""
        test_cases = [([], []), ([1], [5]), ([1, 2, 3], [5, 10, 15])]
        self._test_function_on_cases(a1.five_times_list, test_cases)

    def test_five_times_list_use_list_comp(self):
        """Provided test for five_times_list on list comprehension."""
        five_times_list_ast = ast.parse(inspect.getsource(a1.five_times_list))
        used_list_comp = any(
            type(node) is ast.ListComp for node in
            ast.walk(five_times_list_ast))
        self.assertTrue(
            used_list_comp, "Your did not use list comprehension when "
                            "implementing five_times_list!")
