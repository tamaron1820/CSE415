...
class PlotMultiBPOneVsAll(PlotBinaryPerceptron):
    ...
    def __init__(self, bp, plot_all=True, n_epochs=50):
        ...
        # you can choose a pair of "one-vs-one" by selecting a variable below
        self.CLASSES=(0,1)
        #self.CLASSES=(1,2)
        #self.CLASSES=(0,2)
    ...
    def plot_separator(self, w0, w1, w2):
        ...
        if self.PLOT_ALL:
            plt.plot([self.X_MIN, self.X_MAX], [y1, y2], label='Final Separator'.format(i=self.PLOTLINE_COUNT))
        else:
    ...
    def train(self, verbose=False):
        ...
        """
        self.read_data()
        self.TRAINING_DATA_BINARY=[]
        
        for j in range(len(self.TRAINING_DATA)):
            if self.TRAINING_DATA[j][-1] == self.CLASSES[0]:
                self.TRAINING_DATA_BINARY.append([self.TRAINING_DATA[j][0],self.TRAINING_DATA[j][1],+1])
            elif self.TRAINING_DATA[j][-1] == self.CLASSES[1]:
                self.TRAINING_DATA_BINARY.append([self.TRAINING_DATA[j][0],self.TRAINING_DATA[j][1],-1])
        self.plot_2d_points(self.TRAINING_DATA_BINARY)

        for i in range(self.MAX_EPOCHS):
            changed_count = self.bp.train_for_an_epoch(self.TRAINING_DATA_BINARY)
            if changed_count == 0:
    ...
    def plot(self):
        ...
        """
        plt.title("Plot between classes 0 and 1")
